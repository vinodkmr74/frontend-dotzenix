import React from "react";
import Service_hero from "../../../component/Service_hero";
import { serviceData } from "../../../../data/serviceData";
import Technologies from "../../../slider/Technologies";
import { ImArrowRight2 } from "react-icons/im";
import Dev_process from "../../../component/Dev_process";
import Cta from "../../../component/Cta";
import Service_abouts from "../../../component/Service_abouts";
import Service_img_card from "../../../component/Service_img_card";
export default function Wordpress() {
  return (
      <>
          <Service_hero
            heading={"WordPress Development"}
            text={
              "We build modern, responsive, and user-friendly WordPress websites with custom design, secure functionality, and high-performance solutions to help businesses grow their online presence and customer engagement."
            }
            image={"image/wordpress.png"}
            btn={"Get start"}
          />
    
          <Service_abouts
            heading={"What is WordPress Development? "}
            text={
              "WordPress Development is the process of designing, developing, and customizing websites using the WordPress platform to create responsive, secure, and user-friendly websites. It helps businesses manage content, improve online presence, and deliver better digital experiences."
            }
            image={"image/wordpress.png"}
            title={"Transforming Ideas Into Powerful Digital Solutions"}
            subtext={
              "We create modern, scalable, and user-friendly applications with advanced technology and robust architecture that help businesses grow."
            }
          />
    
          <div className="why-chose">
            <div className="why-chose-heading">
              <h1>Why Choose Our WordPress Development Development Services?</h1>
            </div>
            {serviceData.whyword.map((item, index) => (
              <div className="why-chose-in" key={index}>
                <div className="why-heding">
                  <ImArrowRight2 className="arro-icon" />
                  <p>
                    <span>{item.title} : </span> {item.subtext}
                  </p>
                </div>
              </div>
            ))}
          </div>
    
          <div className="our_frontend_service">
            <div className="our_fron_service_in">
              <div className="our_frontend_service_text">
                <h1>Our WordPress Developmen Services</h1>
              </div>
    
              <div className="our_frontend_service_card_main">
                {serviceData.wordcard.map((item, index) => (
                  <div className="our_frontend_service_card" key={index}>
                    <div className="img-icon"></div>
                    <h1>{item.heading}</h1>
                    <p>{item.text}</p>
                    <img src={item.image} alt="" />
                  </div>
                ))}
              </div>
            </div>
          </div>
    
          <div className="why-chose">
            <div className="why-chose-heading">
              <h1>Technologies We Use in Wordpress Development</h1>
            </div>
            {serviceData.wordtech.map((item, index) => (
              <div className="why-chose-in" key={index}>
                <div className="why-heding">
                  <ImArrowRight2 className="arro-icon" />
                  <p>
                    <span>{item.heading} : </span> {item.text}
                  </p>
                </div>
              </div>
            ))}
          </div>
    
          <div className="frontend_slider">
            <div className="frontend_tech_heading">
              <h1>Technologies We Use</h1>
            </div>
            <div className="frontend_slider_in">
              <Technologies
                html={"image/css.jpg"}
                css={"image/python.png"}
                javascript={"image/mongo.png"}
                react={"image/html.png"}
                bootstrap={"image/shopify.png"}
                nextjs={"image/reactjs.png"}
              />
            </div>
          </div>
    
          <Dev_process />
          <Service_img_card />
          <Cta />
        </>
  )
}
