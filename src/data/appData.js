import { FcMissedCall } from "react-icons/fc";

export const appData = {
  contact: [
    {
      id: 1,
      email: "dotzenix96@gmail.com",
      mob: "6206030752",
      address: "noida",
      linkedin: "https://www.linkedin.com/in/vinod-kumar210196/",
    },
  ],
  company: [
    {
      id: 1,
      company: "About Us",
      company_path: "/about",
    },
    {
      id: 2,
      company: "Careers",
      company_path: "/careers",
    },
    {
      id: 3,
      company: "Blogs",
      company_path: "/blogs",
    },
  ],

    Industries: [
    {
      id: 1,
      Industries: "Healthcare",
      industries_path: "/healthcare",
    },
    {
      id: 2,
         Industries: "Education",
      industries_path: "/education",
    },
    {
      id: 3,
    Industries: "Ecommerce",
      industries_path: "/ecommerce",
    },
    {
      id: 4,
       Industries: "Menufacturing",
      industries_path: "/menufacturing",
    },
    {
      id: 5,
      Industries: "Realstate",
      industries_path: "/realstate",
    },
  ],

  Product: [
    {
      id: 1,
      product: "CRM",
      product_path: "/crm",
    },
    {
      id: 2,
      product: "ERP",
      product_path: "/erp",
    },
    {
      id: 3,
      product: "HRM",
      product_path: "/hrm",
    },
    {
      id: 4,
      product: "AI Solutions",
      product_path: "/aisolutions",
    },
    {
      id: 5,
      product: "Business Analytics",  
      product_path: "/businessanalytics",
    },
    {
      id: 6,
      product: "Website Builder",
      product_path: "/websitebuilder",
    },
  ],
  menuproduct: [
    {
      id: 1,
      title: "Web Development",
      items: [
        { id: 1, name: "Frontend", path: "/frontend" },
        {
          id: 2,
          name: "Backend",
          path: "/backend",
        },
        {
          id: 3,
          name: "Database",
          path: "/database",
        },
        { id: 4, name: "UI/UX", path: "/ui-ux" },
      ],
    },
    {
      id: 2,
      title: "Mobile App Development",
      items: [
        { id: 1, name: "Android App", path: "/android-app" },
        { id: 2, name: "IOS App", path: "/ios-app" },
        {
          id: 3,
          name: "OTT App Development",
          path: "/ott-app-development",
        },
      ],
    },
    {
      id: 3,
      title: "Ecommerce Development",
      items: [
        { id: 1, name: "Shopify", path: "/shopify" },
        {
          id: 3,
          name: "Prestashop",
          path: "/prestashop",
        },
      
        {
          id: 6,
          name: "WordPress",
          path: "/wordpress",
        },
        {
          id: 7,
          name: "Custom Ecommerce",
          path: "/custom-ecommerce",
        },
      ],
    },
    {
      id: 4,
      title: "Digital Marketing",
      items: [
        { id: 1, name: "SEO", path: "/seo" },
        { id: 1, name: "SMO", path: "/smo" },
        {
          id: 3,
          name: "Content Marketing",
          path: "/content-marketing",
        },
        {
          id: 6,
          name: "Affiliate Marketing",
          path: "/affiliate-marketing",
        },
      ],
    },
    {
      id: 6,
      title: "Digital Transformation",
      items: [
        { id: 1, name: "DevOps", path: "/devops" },
        {
          id: 4,
          name: "AI and ML",
          path: "/ai-ml",
        },
        {
          id: 6,
          name: "Cloud Engineering",
          path: "/cloud-engineering",
        },
      ],
    },
  ],
  sliderData: [
    {
      id: 1,
      image: "image/web_slider.png",
      text: "#2 Web Develoments Turning Ideas into Powerful Digital Experiences",
    },
    {
      id: 2,
      image: "image/Ai_slider.png",
      text: "#1 Artificial Intelligence and  Machine Learning",
    },
    {
      id: 3,
      image: "image/App_dev_slider.png",
      text: "#3 App Development Building Powerful Mobile Apps for the Digital Future",
    },
    {
      id: 4,
      image: "image/Business_slider2.png",
      text: "#4 Business Development Driving Growth Through Smart Digital Strategies",
    },
  ],
  TextData: [
    {
      id: 1,
      hedding:
        "Empowering Businesses Through Smart Digital Engineering and Excellence for a Smarter Tomorrow",
      subhedding:
        "At DotZenix Technologies, we design and develop scalable, secure, and high-performance digital solutions that help businesses innovate, compete, and grow in today's fast-changing digital landscape.",
    },
    {
      id: 2,
      hedding: "Our Services",
      subhedding:
        "Delivering enterprise-grade software Development trusted by growing businesses and established brands.",
    },
    {
      id: 3,
      hedding: "Our Expertise, Your Success",
      subhedding:
        "Offring modern technology solutions to transform your business.",
    },
    {
      id: 4,
      hedding: "Explore Our Latest Blogs & Insights",
      subhedding:
        "Learn about new technologies, industry trends, and important updates through our blogs and articles.",
    },
    {
      id: 5,
      hedding: "Turn Your Vision into Digital Success",
      subhedding:
        "Share your ideas with us and let our experts craft efficient, customized digital marketing solutions that drive real results.",
    },
  ],
  servicesCard: [
    {
      id: 1,
      iconpath: "image/web-icon-developer.jpg",
      title: "Web Development",
      subitems: [
        {
          id: 2,
          subitem: "Frontend",
        },
        {
          id: 3,
          subitem: "UI/UX",
        },
        {
          id: 4,
          subitem: "Java",
        },
        {
          id: 5,
          subitem: "Python",
        },
        {
          id: 6,
          subitem: "PHP",
        },
        {
          id: 7,
          subitem: "Node.js",
        },
        {
          id: 8,
          subitem: "MySQL",
        },
      ],
    },
    {
      id: 2,
      iconpath: "image/app-development.jpg",
      title: "Mobile App Development",
      subitems: [
        {
          id: 9,
          subitem: "Android App",
        },
        {
          id: 10,
          subitem: "IOS App",
        },
        {
          id: 11,
          subitem: "Flutter App",
        },
        {
          id: 12,
          subitem: "React Native",
        },
        {
          id: 13,
          subitem: "Hybrid App",
        },
        {
          id: 14,
          subitem: "OTT App Development",
        },
      ],
    },
    {
      id: 3,
      iconpath: "image/13137305.png",
      title: "Digital Marketing",
      subitems: [
        {
          id: 22,
          subitem: "SEO",
        },
        {
          id: 23,
          subitem: "SMO",
        },
        {
          id: 24,
          subitem: "Content Marketing",
        },
        {
          id: 25,
          subitem: "Paid Media Marketing",
        },
        {
          id: 26,
          subitem: "Video Marketing",
        },
        {
          id: 27,
          subitem: "Affiliate Marketing",
        },
        {
          id: 28,
          subitem: "Ecommerce SEO",
        },
      ],
    },
    {
      id: 4,
      iconpath: "image/digitalization-icon_1186366-109534.jpg",
      title: "Digital Transformation",
      subitems: [
        {
          id: 34,
          subitem: "IoT",
        },
        {
          id: 35,
          subitem: "DevOps",
        },
        // {
        //     "id": 36,
        //     "subitem": "Data Forecasting"
        // },
        {
          id: 37,
          subitem: "AI and ML",
        },
        {
          id: 38,
          subitem: "Blockchain",
        },
        {
          id: 39,
          subitem: "Cloud Engineering",
        },
        {
          id: 40,
          subitem: "Big Data",
        },
      ],
    },
  ],
  productCard: [
    {
      id: 1,
      image: "image/download-hrm.jpg",
      title: "HRM",
      path: "/hrm",
    },
    {
      id: 2,
      image: "image/erp.avif",
      title: "ERP",
      path: "/erp",
    },
    {
      id: 3,
      image: "image/images-crm.jpg",
      title: "CRM",
      path: "/crm",
    },
    {
      id: 5,
      image: "image/artificial-intelligen.jpg",
      title: "AI Solutions",
      path: "/aisolutions",
    },
    {
      id: 6,
      image: "image/business.jpg",
      title: "Business Analytics",
      path: "/businessanalytics",
    },
    {
      id: 7,
      image: "image/web-icon-developer.jpg",
      title: "Website Builder",
      path: "/websitebuilder",
    },
  ],
  missionvission: [
    {
      id: 1,
      heading: "Mission & Vision",
      image: "image/target.png",
      title: "Mission and Vision",
      subtext:
        "To empower businesses by delivering innovative solutions that help our clients build a successful and future-ready world.   Delivering next-level technology to drive innovation and business growth.",
    },
    {
      id: 2,
      heading: "Growing Companies & Businesses",
      image: "image/rise-glowing-arrow.jpg",
      title: "Helping businesses scale with smart technology.",
      subtext:
        "DotZenix Technologies has been a global IT consulting company with more than 50+ employees, helping businesses grow through innovative solutions.",
    },
    {
      id: 3,
      heading: "Technology Partners",
      image: "image/businessman-handshake.avif",
      title: "Our Technology Partners",
      subtext:
        "We collaborate with world-class technology partners to deliver advanced, scalable solutions that enable exceptional and personalized customer experiences.",
    },
  ],
  Satisfied: [
    {
      id: 5,
      text: "Satisfied Clients",
      number: 100,
    },
    {
      id: 6,
      text: "INDUSTRY EXPERTISE",
      number: 5,
    },
    {
      id: 7,
      text: "Awards & Achievement",
      number: 15,
    },
    {
      id: 8,
      text: "Finished Projects",
      number: 150,
    },
    {
      id: 9,
      text: "Skilled Experts",
      number: 40,
    },
  ],
  ourexpertise: [
    {
      id: 3,
      image: "image/seo.png",
      tital: "Search Engine Optimization (SEO)",
      text: "Search Engine Optimization (SEO) Boost your online visibility with our SEO expertise. We will optimize your website to rank higher in search engines and attract more organic traffic.",
      buttonlink: "/seo",
    },
    {
      id: 4,
      image: "image/app-deve.png",
      tital: "Mobile App Development Services",
      text: "Turn your ideas into powerful mobile experiences. We create intuitive, scalable apps that deliver smooth performance and keep users engaged.",
      buttonlink: "/android-app",
    },
    {
      id: 5,
      image: "image/digital.png",
      tital: "Social Media Marketing",
      text: "Grow your brand’s presence on social platforms with strategic marketing. We help you engage your audience, build trust, and generate quality leads.",
      buttonlink: "/paid-media-marketing",
    },
    {
      id: 6,
      image: "image/product-dev.png",
      tital: "Product Development",
      text: "Transform your ideas into powerful digital products. We design and develop scalable, high-quality solutions tailored to your business needs.",
      buttonlink: "/salesforce-product-development",
    },
    {
      id: 7,
      image: "image/devops-logo.webp",
      tital: "DevOps",
      text: "Streamline your development and operations with our DevOps solutions. We automate workflows, improve deployment speed, and ensure reliable system performance.",
      buttonlink: "/devops",
    },
    {
      id: 8,
      image: "image/bigcommerce-logo-icon.png",
      tital: "E-Commerce Development",
      text: "Launch and grow your online store with BigCommerce solutions designed for performance, flexibility, and seamless user experience.",
      buttonlink: "/bigcommerce",
    },
  ],

  exploreour: [
    {
      id: 5,
      image: "image/codeimage.png",
      tital: "Why Choose AI, ML, and Java & Python Development Services?",
      text: "Java and Python are powerful programming languages used to build secure, scalable, and high-performance applications  They help businesses develop web applications, AI solutions, enterprise software, and modern digital platforms efficiently. Python is widely used for Artificial Intelligence, Machine Learning, and Data Analytics because of powerful libraries like TensorFlow, Pandas, and Scikit-Learn  It helps companies build AI chatbots, automation tools, and predictive systems.",
    },
    {
      id: 6,
      image: "image/software-developer-ai.jpg",
      tital: "Why DotZenix Technologies is Leading AI & ML Company in Noida",
      text: "",
    },
    {
      id: 7,
      image: "image/web.png",
      tital: "Why Choose Java & Python Development Services?",
      text: "",
    },
  ],

  about_mission: [
    {
      id: 1,
      heading: "Our Mission",
      text: "Our mission is to deliver high-quality, scalable, and innovative digital solutions that help businesses grow and succeed in the modern world. We focus on creating reliable and user-friendly applications using the latest technologies.",
      image: "image/mission.avif",
    },
    {
      id: 2,
      heading: "Our vision",
      text: "Our vision is to become a trusted technology partner, recognized for delivering innovative solutions and helping businesses achieve long-term digital success.",
      image: "image/vision.jpg",
    },

    {
      id: 3,
      heading: "Our Services",
      text: "We provide design and Web development, software development, AI development, and custom software solutions. By leveraging modern technologies, we deliver secure, scalable, and high-performance digital solutions that support business growth.",
      image: "image/images_service.jpg",
    },
  ],
};
